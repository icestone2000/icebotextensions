"""
将轮胎报价文本发到当前会话：个人微信 / 企业微信第三方 / 闲鱼。
逻辑对齐 express_price_calc/message_sender.py。
"""
...
def send_tire_quote_to_customer(
    db: Session,
    ...
    recipient_id = str(recipient_id_override).strip() if recipient_id_override else ""
    if not recipient_id:
    ...
    if not recipient_id:
        return {
            "ok": False,
            "channel": "",
            "detail": "缺少接收方：请提供 recipient_id 或确保 request 含 group_id/group_name",
        }

    config = resolve_wechat_config(db, request)

    if not config:
        app_logger.warning("[tire_price_query] 无法解析微信配置，回退个人微信服务")
        return _send_personal_wechat(recipient_id, text)

    account_type = (config.account_type or "").strip()
    app_logger.info(
        "[tire_price_query] send resolved config_id=%s account_type=%s recipient=%s",
        config.id,
        account_type,
        recipient_id,
    )
...
def _get_wecom_instance(db: Session, config: WeChatConfig) -> Optional[WeComThirdPartyInstance]:
    try:
        ...
        app_logger.error("[tire_price_query] _get_wecom_instance: %s", e, exc_info=True)
        return None
...
def _send_personal_wechat(recipient_id: str, text: str) -> Dict[str, Any]:
    try:
        ...
        return {
            ...
            "detail": send_result.error or "发送失败",
        }
    except RuntimeError as e:
        return {"ok": False, "channel": "wechat_personal", "detail": str(e)}
    except Exception as e:
        app_logger.exception("[tire_price_query] personal wechat send: %s", e)
        return {"ok": False, "channel": "wechat_personal", "detail": str(e)}
...
def _send_wecom(
    ...
    try:
        ...
        app_logger.exception("[tire_price_query] wecom send: %s", e)
...
def _send_xianyu(
    ...
    try:
        ...
        db_group_id = (
            str(request.group_id or "").strip() or str(group_info.get("id") or "").strip()
        )
        if db_group_id:
            XianyuService.save_outgoing_text_message(config.id, db_group_id, text)
        else:
            app_logger.warning(
                "[tire_price_query] 闲鱼消息已发送但未落库：缺少 request.group_id 与 context.group_info.id"
            )
        return {"ok": True, "channel": "xianyu", "detail": "已发送"}
    except Exception as e:
        app_logger.exception("[tire_price_query] xianyu send: %s", e)
